...
const TRANSLATIONS = {
    'en': {
        ...
        'sortHint': 'Click to sort by this time - ascending, descending, then back to your order. The filled triangle shows which way',
        'tags': 'Tags',  
        ...
        'aboutAlwaysOnTopDesc': 'On by default, so the list does not disappear behind your work. The pin lives in the collapsed side strip, where the window is actually sitting over other work; it is filled while pinned. Expanded, press Ctrl+M to reach it',
    ...
    'ko': {
        ...
        'startTime': '시작 시간',
        'targetTime': '목표 시간',
        'sortHint': '누르면 이 시간으로 정렬합니다 - 오름차순, 내림차순, 그리고 원래 순서. 속이 찬 삼각형이 지금 방향입니다',
        'tags': '태그',
        'taskContent': '작업 내용',
        'status': '상태',
        // Tooltips
        ...
        'aboutAlwaysOnTopDesc': '기본으로 켜져 있어 목록이 하던 일 뒤로 사라지지 않는다. 핀은 접힘 스트립에 있다 - 창이 실제로 다른 작업 위에 얹혀 있는 상태가 거기다. 고정된 동안 핀에 색이 차 있고, 펼친 상태에서는 Ctrl+M 으로 접어서 닿는다',
    ...
    'zh': {
        ...
        'startTime': '开始时间',
        'targetTime': '目标时间',
        'sortHint': '点击可按此时间排序——升序、降序，再回到原顺序。实心三角表示当前方向',
        'tags': '标签',
        'taskContent': '任务内容',
        'status': '状态',
        // Tooltips
        ...
        'aboutAlwaysOnTopDesc': '默认开启，使列表不会被其他窗口盖住。图钉位于折叠侧边条中——那才是窗口真正压在其他工作之上的状态；置顶期间图钉是填充的。展开时按 Ctrl+M 即可到达',
    ...
    'ja': {
        ...
        'startTime': '開始時刻',
        'targetTime': '目標時刻',
        'sortHint': '押すとこの時刻で並べ替える - 昇順、降順、そして元の順。塗られた三角が今の向き',
        'tags': 'タグ',
        'taskContent': 'タスク内容',
        'status': 'ステータス',
        // Tooltips
        ...
        'aboutAlwaysOnTopDesc': '既定で有効。リストが作業の裏に消えない。ピンは折りたたみストリップにある - ウィンドウが実際に他の作業の上に乗っているのがその状態だ。最前面の間はピンが塗られ、展開中は Ctrl+M で辿り着ける',
    ...
    'es': {
        ...
        'sortHint': 'Pulse para ordenar por esta hora: ascendente, descendente y de vuelta. El triángulo relleno indica la dirección',
        'tags': 'Etiquetas',
        ...
        'aboutAlwaysOnTopDesc': 'Activado de forma predeterminada, para que la lista no desaparezca detrás de su trabajo. La chincheta está en la tira lateral contraída, que es cuando la ventana de verdad se apoya sobre otro trabajo; se rellena mientras está fijada. Expandido, pulse Ctrl+M para llegar',
