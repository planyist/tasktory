...
- **An advanced row goes to the end of the arrangement.** `#` is the order of the work in front of you, and a row whose target just jumped a week is not in front of you. Leaving it in place made repeats the only thing you can complete that does not get out of the way — a one-off vanishes and frees its slot, and to the person ticking the box both were equally done. The cost is real and accepted: a repeat deliberately placed at #1 has to be put back there, one press of the up button. Several completed repeats pile up in the order they were finished; ordering that pile by next occurrence is what the sort headers already do, and mixing dates into `#` would blur what it means.
...
- One file per day under `<userData>/logs/`, named `YYYY-MM-DD.tsv` by **local** date. Columns: `TIMESTAMP ACTION STATUS TASK_ID START_TIME TARGET_TIME TAGS CONTENT ATTACHMENTS COMPLETED_AT NOTE OUTPUTS`. Every one of the last three was appended rather than inserted, because the readers index by position — a column added earlier would shift every line already on disk.
...
- **The collapsed window height is measured, never estimated.** `resizeCollapsedWindow` reads the strip's content *after* rendering. Counting rows and multiplying needed its constants re-tuned every time the content changed, and the mini grid broke it immediately — the day's list was pushed off the bottom of the window. `collapsedRowCount()` survives only as a fallback for jsdom, which has no layout.

  **Measuring the layout box does not measure the content.** `collapsedMiniLayout` is `height: 100%`, so its `scrollHeight` is the window's height, not the strip's. That made the "measured" height a function of the window it was setting: each redraw asked for 29px less than the last, and a three-item strip started at 837px and crept down forever while its content stayed 131px. `collapsedContentHeight()` now takes the stretch out first — `height: auto` on the layout, `flex: 0 0 auto` on the list — measures, and puts both back. Summing the children instead is wrong in a quieter way: `offsetHeight` excludes margins, so the expand button's 6px and each row's 1px go missing, and the last line is cut off by however many you forget.

  **Three things stack above the content and all three must be added.** The layout, the chrome around it inside the window, and the window frame. The renderer can measure the first two (`innerHeight - layout.clientHeight`) but **not the frame**: `window.outerHeight` still reports the pre-collapse size, answering 728px for a 39px frame, which sent the strip straight to its cap. `main.js` adds the frame, from `getBounds().height - getContentSize()[1]`, because only the window knows it.

  **It stops at half the work area, and the list scrolls.** A sticky note taller than half the screen is not a note any more. The header and mini calendar stay put — they say which day you are looking at — so `.collapsed-task-list` is the only thing that scrolls.
...
- **The strip goes back where you put it, until you restart.** Moving or resizing it while collapsed is remembered in a plain variable, never in `localStorage`. That is deliberate: if the strip ends up somewhere wrong or some size you did not intend, closing and reopening the app has to bring it back to the corner. A stored value would carry the problem across restarts, and the app has no screen for editing it.
  ...
  **`moved` never fires for the strip, so listening to it alone recorded nothing.** The window is dragged by `.drag-bar`'s `-webkit-app-region: drag`, and a move made that way raises `move` — measured at six times for one drag — and then no `moved` at all. The feature shipped watching `moved`/`resized`, so every drag was invisible and the strip went back to the corner on the next collapse, which is exactly what it was built not to do. All four events are bound now.

  **A test that emits the event itself proves nothing here.** The original check did `win.setBounds(...)` then `win.emit('moved')`, and passed against code that could never work in the app. `check-window.js` drives a real CDP mouse drag on `.drag-bar` instead; reverted to `moved`/`resized` only, it fails.

  Two traps come with the continuous events. `move` and `resize` fire for OS-driven moves as well as yours, which is why `disrupted` already exists — the same guard covers this. And they fire again right after *we* place the strip, so `placingCollapsed` suppresses that; without it the height we just computed becomes "the height the user chose", and the strip never fits its content again.

  `collapsedStripBounds()` holds the whole decision and is exported, so the caps, the corner and the remembered position are testable without an Electron runtime — the same shape as `boundsToRestore`.

- **The task edit dialog fits one screen at the default window size**, and keeps its scrollbar for when it does not. Measured, it overflowed by 39px — so the Save button sat below the fold on a freshly installed app, which reads as the dialog being broken rather than tall. Start Time and Target Time now share a row (`.form-row`), which is 59px and covers it with room to spare. They are a pair and read better side by side anyway; at 179px each, the `YYYY-MM-DD HH:mm` mask has 31px of slack. `min-width: 0` on the two groups is load-bearing — without it a flex item refuses to shrink below its content and both fields push out of the dialog on a narrow window. `check:ui` measures the overflow, the shared row and that `overflow-y` is still `auto`.
...
- **A refusal is a bubble floating under the field it refused — never in an OS dialog, and never in the layout.** `refuse(message, fieldId)` is the only way a validation message is shown; every branch of `saveTask`, the completion dialog and the tag presets names the field it is complaining about, and `clearRefusals()` wipes the last answer whenever a dialog opens or Save is pressed again.

  **It is `position: absolute`, because a message in the flow pushes the dialog around.** The first version was a plain line under the input, which moved every field below it down 22px the moment it appeared — reported as the dialog twisting, and fair: nothing you did asked for the form to rearrange itself. Out of the flow there is nothing to push. `check:ui` measures the Save button, the start-time field and the content box before and after a refusal and fails if any of the three moves.

  Two things follow from floating. It carries its own background and white text, so it reads the same in both themes and has no `body.dark-mode` twin — the one place in this file where a hardcoded colour needs no counterpart. And it **covers the field below**, so it retreats on the first `input` or `keydown` in the field it is attached to; date fields cancel `keydown` and write the value themselves, so listening for `input` alone would leave the bubble sitting there while you type.

  It was `alert()`, and `alert()` does not give the caret back. Closing it leaves focus on the Save button that opened it, so typing goes nowhere until the window is activated from another application — reported as *the input stops working*. Calling `field.focus()` after the alert does **not** fix it: `document.activeElement` is then correct and the keystrokes still do not arrive. Removing the dialog removes the problem, because there is no longer anything to take the focus.

  **This is also the one bug on this list I could not verify while it used `alert()`.** A native dialog cannot be dismissed from a probe — driving someone else's window is the thing this file already forbids — so the check stubbed `window.alert` out and measured a path that never showed a dialog at all. It passed, and the bug shipped. Two other stubbed probes did the same thing the same day (`win.emit('moved')`, and a mask test asserting `maskWrite` returns null past the end). **If a probe has to replace the thing under test, it is not testing it.**

  The in-page message is verifiable end-to-end, which is why the fix is also the cheaper thing to keep honest: `renderer-dom.test.js` reads `.field-error` out of the DOM, and a real-window probe types a real key straight after a refusal.

  Export, import and the browser-storage failure keep `alert()`. They have no field to sit under, and none of them is followed by typing.
...
- **The `#` column is the order the user arranged**, not a row count — the up/down buttons and the position field write it. The one place the app moves a row itself is a repeating task advancing to its next occurrence, which sends it to the end; see Recurring Tasks for why. Sorting is therefore a *way of looking*, never a change: `sortForDisplay()` runs on the way to the screen, `this.tasks` is untouched, nothing is saved, and a restart comes back unsorted.
