...
- **Double-click a row to edit, and it does nothing to the selection.** The toggle waits `DOUBLE_CLICK_MS` (130ms) to see whether a second click arrives; `dblclick` cancels it. Two earlier attempts were both wrong in ways worth not repeating: letting both clicks toggle relies on an even number cancelling out, but the select-then-deselect is visible; skipping the second click via `e.detail` alone means the first toggle has already happened. Only holding the first click gives "either select, or edit — not both".
  ...
  **The wait can be short because a slow double-click is undone rather than missed.** Every ordinary selection pays this delay, so it is ours, not the OS threshold (500ms here). At 130ms a leisurely double-click gets past it — the timer records `pendingRowToggle`, and `dblclick` reverses that toggle before opening the editor. `e.detail <= 1` is what tells the click handler a *new* run of clicks has started and the old marker can be dropped; without that check the second click of a slow double-click erases the marker it is about to need. Measured at 30ms, 100ms and 250ms gaps: selection untouched, editor opened once
