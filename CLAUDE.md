...
- **Completed view** (`viewMode: 'completed'`): the third view, and it reads the **TSV log**, never `tasks.json`.

  The question "should completed rows live in the JSON instead" came up and was settled by measuring. Three years of heavy use (1,095 files, 7.5MB) reads in 215ms for *all* of it and 7ms for the thirty days the screen actually shows; the same rows in one JSON parse in 21ms. JSON wins on that one number and loses everywhere else: `tasks.json` is parsed whole at every launch and carried whole in every backup, so its cost is paid always rather than when the screen opens. That is the same reasoning that deleted the kept `completed: true` rows in the first place.

  The decisive fact is not performance, though. **Thirteen months of completions already exist only in the TSV** — a JSON store would start empty and would need the TSV reader anyway to show any history at all. And `readCompleted()` already exists and already backs the daily counter and the hover panel, so three surfaces share one parser and cannot disagree.
  ...
  - `get-completed-range(from, to)` walks day keys as strings and calls the existing per-day reader. Walking with `Date` arithmetic would skip or double a day across a DST boundary, which is exactly the class of bug the log's local-date filenames already have to survive.
  - **It sorts by `COMPLETED_AT`, not `TIMESTAMP`.** The column on screen is the completion time, and backdating makes the two disagree — ordering by the one you are not showing reads as broken sorting. Rows written before that column existed fall back to `TIMESTAMP`.
  - The search box filters it. A control that visibly does nothing is worse than a hidden one, which is also why the quick filters are hidden here: they are built from the active tasks' statuses and tags, and every row here is completed.
  - Thirty days at a time, and the range will not walk past today — there is no tomorrow in the log.
