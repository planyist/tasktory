...
- **The date field wears its format as a mask.** Empty, it shows `YYYY-MM-DD HH:mm`; each digit overwrites one placeholder character from the left and the separators are stepped over, so `20260821` lands as `2026-08-21 HH:mm` with the caret at the next slot. `maskParts` reads the same `DATE_TOKENS` table that builds the output and the parsing regex — the mask is the third thing generated from one format string, so a new format cannot be supported by two of the three.
  ...
  **The field is re-rendered from its own contents on every keystroke**, which only works because the placeholder is unambiguous: `maskRead` keeps digits and `A`/`P`, and nothing else in the template is either. Backspace is handled by hand — left to the browser it deletes a separator or a placeholder letter, which the next render puts straight back, so the key appears dead.

  **Focus selects the whole value.** Without it, a field already holding a full date has no room for the first digit and typing silently does nothing — which is what the first version did. Blur clears a field nobody typed into, because a target time is allowed to be empty and a bare template cannot be told apart from a value.
