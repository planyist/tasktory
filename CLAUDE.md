...
- **Completed view**: it reads the **TSV log**, never `tasks.json`, and it is reached from the **daily counter** rather than the view toggle.
  ...
  - **You have to be able to tell you are in it.** The view carries its own heading, and the counter you came through stays lit while it is open. Swapping the content alone leaves no way to know whether the list is filtered or this is different data altogether.
