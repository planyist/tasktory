...
- **Double-click a row to edit, and it does nothing to the selection.** The toggle waits `DOUBLE_CLICK_MS` (220ms) to see whether a second click arrives; `dblclick` cancels it. Two earlier attempts were both wrong in ways worth not repeating: letting both clicks toggle relies on an even number cancelling out, but the select-then-deselect is visible; skipping the second click via `e.detail` means the first toggle has already happened, so a double-click still changes the selection. Only holding the first click gives "either select, or edit — not both", which is what the action means. The wait is ours, not the OS threshold (500ms here): real double-clicks are far quicker, and every ordinary selection pays this delay
...
- **The `#` column is the order the user arranged**, not a row count — the up/down buttons and the position field write it. Sorting is therefore a *way of looking*, never a change: `sortForDisplay()` runs on the way to the screen, `this.tasks` is untouched, nothing is saved, and a restart comes back unsorted.
  ...
  **Sorted shows one triangle, not two.** The earlier version dimmed the unused one, which meant reading the direction off a contrast difference — fine side by side, ambiguous alone. The idle pair says "sortable"; a single triangle says which way. `visibility: hidden` rather than `display: none` keeps the box, so the label does not shift.
