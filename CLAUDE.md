...
- **Always-on-top is a pin, and it lives in the collapsed strip.** It is a state you flip, not a value you configure — opening a dialog to get the window out of your way is backwards. It sits in the strip rather than the expanded header because that is where the window is actually lying over other work; expanded, the window is what you are looking at. Expanded there is no pin at all, and that is the accepted cost: Ctrl+M reaches it in one keystroke.

  **The icon never changes** — that breaks the "show what you get" rule collapse and the view toggle follow, and breaks it deliberately: with those two the question is *what will this do*, but with a pin the question is *is it pinned right now*, so the state has to be the visible thing. Only the tooltip says what pressing will do, and `.pin-btn.active` paints a background because nothing else in that strip carries state.
...
- **Double-click a row to edit.** The first click toggles at once; the second is skipped by reading `e.detail`, the browser's own count of consecutive clicks. That avoids both failure modes: holding the first click back until a double-click can be ruled out freezes every ordinary selection for the OS threshold (500ms here), and letting both clicks toggle leaves a visible select-then-deselect flicker. The row ends up selected, which is what editing wants anyway
...
- **The `#` column is the order the user arranged**, not a row count — the up/down buttons and the position field write it. Sorting is therefore a *way of looking*, never a change: `sortForDisplay()` runs on the way to the screen, `this.tasks` is untouched, nothing is saved, and a restart comes back unsorted.
  ...
  **Direction is shown by filling one triangle, not by colouring one.** Colouring half of a two-glyph indicator needs a second pseudo-element, and it has to be absolutely positioned — which does not work here, because these headers are centre-aligned, so `right: 6px` lands the arrow at the cell edge instead of beside the label. That was the first attempt and the blue triangle floated off on its own. Swapping the content of one `::after` (`▵▿` → `▴▿` or `▵▾`) has no such problem, and all three states occupy the same box so nothing shifts. The glyph carries its own `font-size`: the header text is 11px and an inherited arrow disappears at that size.
