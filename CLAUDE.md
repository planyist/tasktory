...
- **Calendar view** (`viewMode`, persisted): a month grid, Monday-first, alongside the list. A list answers "what is there", a calendar answers "when does it pile up". The toggle sits with the quick filters under the search box, and its icon shows what you *get*, not what you have — the same rule the collapse button follows. **View-only on purpose** — no selection, no editing, nothing clickable inside a cell; the list does all of that.
- A task appears on *every* day it spans, because work started Monday and due Wednesday is in hand on Tuesday too. The time on a chip is the **target** time, and days sort by it — showing the start time while sorting by something else made the visible times look out of order. Tasks with no target have no length to draw, so they sit at the top of their start day with no time, where an all-day entry goes.
- **`grid-auto-rows` must be `max-content`.** With a definite container height Chromium collapses both `auto` and `minmax(60px, max-content)` to exactly the 60px floor, which trapped a busy day inside its cell: the chips were squeezed flat (text clipped, looking like a smaller font) and every cell grew its own scrollbar. `max-content` lets the week's busiest day set the row height, so there is one scroll surface instead of thirty. `.cal-day` must not set `overflow: hidden` either — that zeroes a grid item's automatic minimum size and brings the whole problem back. `.cal-chip` needs `flex-shrink: 0` for the same reason.
- Collapsed calendar narrows to one day — 150px cannot hold seven columns. It shows today, or the next day with work if today is clear, or the most recent past day, so the always-visible strip never goes blank while work remains. `collapsedRowCount()` sizes the window to what is actually drawn, not to every active task. Cells are built as an HTML string, so `escapeHtml` is not optional
...
- **The About dialog is the manual.** Every feature has to be reachable from it — views, search and filters, repeat rules, statuses, shortcuts. It went stale once (it still described the Actions column and a bell button in the row long after both were removed), which is worse than no help at all. The version there reads from `app.getVersion()` over the `get-app-version` IPC; it used to be typed into the HTML by hand and sat at 0.6.4 for three releases
